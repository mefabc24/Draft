...
public sealed class SnapshotCleanupService
{
    ...
    private static DateTimeOffset GetSnapshotTimestamp(string snapshotFolder)
    {
        ...
        ManualSaveSnapshotMetadata? manualMetadata = ManualSaveSnapshotService.TryReadMetadata(
            Path.Combine(snapshotFolder, "metadata.json"));
        AutosaveSnapshotMetadata? autosaveMetadata = AutosaveSnapshotService.TryReadMetadata(
